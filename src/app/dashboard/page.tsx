...
export default function AIChat() {
  ...
  const fetchChatHistory = async () => {
    try {
      ...
      if (data.chats && Array.isArray(data.chats)) {
        // Only take the last 10 chats to limit the display
        const recentChats = data.chats.slice(-10);
        recentChats.forEach((chat: { id: any; prompt: any; created_at: string | number | Date; action: any; response: any; updated_at: string | number | Date; }) => {
          // Add user message
  ...
  const sendRequest = async (userMessage: any) => {
    try {
      ...
      // After getting response, reload chat history from database
      setTimeout(() => {
        fetchChatHistory();
      }, 1000); // Small delay to ensure backend has saved the data
  ...
  const handleQuickAction = async (action: string, prompt: string) => {
    try {
      ...
      // Reload chat history after quick action
      setTimeout(() => {
        fetchChatHistory();
      }, 1000);
  ...
  const getActionBadge = (action: string) => {
    ...
    const actionColors: { [key: string]: string } = {
      ...
      'world news': 'bg-orange-600',
      'megatrends': 'bg-indigo-600',
      'best currencies': 'bg-yellow-600',
      'economic cycle': 'bg-teal-600',
      'compare stocks': 'bg-pink-600',
      'upcoming listings': 'bg-cyan-600',
      'general': 'bg-gray-600'
    };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-black text-gray-100">
      ...
      <div className="flex flex-col flex-1">
        {/* Messages Container */}
        <div className="flex-1 overflow-y-auto" key={forceUpdate} ref={messagesContainerRef}>
          <div className="mx-auto max-w-4xl p-6 space-y-4 bg-black/20 rounded-xl border border-white/10 backdrop-blur-sm shadow-lg">
            {messages.length === 0 ? (
              ...
              messages.map((msg) => {
                const isLongAIResponse = msg.type === 'bot' && 
                  (msg.action === 'analyze stock' || msg.action === 'analyze portfolio') && 
                  ...
                  <div className="absolute mt-2 bg-gray-800 border border-gray-700 rounded-lg shadow-xl z-20 overflow-hidden" style={{ minWidth: '320px', maxWidth: '420px', width: 'max-content' }}>
                    <div className="p-4">
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-end gap-2 pt-1 border-t border-gray-00/50">
                          <button
                            className="px-3 py-1 text-sm bg-gray-500/80 text-gray-200 border border-gray-800 rounded-md hover:bg-gray-600 hover:text-white transition-all duration-100 focus:outline-none focus:ring-1 focus:ring-gray-400"
                          ...
                          <button
                            ...
                            disabled={
                              ...
                              !compareA.trim() || 
                              !compareB.trim() || 
                              compareA.trim().toUpperCase() === compareB.trim().toUpperCase()
                            }
